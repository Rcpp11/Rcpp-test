context( "Vector" )
sourceCpp( "cpp/Vector.cpp", env = environment() )
...
test_that( "Vector support STL-like methods", {
    expect_equal( integer_push_back(1:4), 1:5)
    ...
    expect_equal( integer_push_back(x), target)

    expect_equal( integer_push_front(1:4), c(5L,1:4))
    ...
    expect_equal( integer_push_front(x), target)

    expect_equal( integer_insert(1:4), c(5L,1L, 7L, 2:4))
    ...
    expect_equal( integer_insert(x), target)
    expect_equal( integer_erase(1:4), c(1L, 2L, 4L))
    ...
    expect_equal( integer_erase(x), target)

    x <- y <- 1:10
    names(y) <- letters[1:10]
    res <- integer_erase_range( x, y )
    expect_equal( res[[1L]], c(1:5, 10L) )

    z <- y[-(6:9)]
    expect_equal( res[[2L]], z )

    x <- y <- 1:10
    names(y) <- letters[1:10]
    res <- integer_erase_range_2( x, y )
    expect_equal( res[[1L]], 1L )
    expect_equal( res[[2L]], c("a" = 1L ) )

    x <- y <- as.list(1:10)
    names(y) <- letters[1:10]
    res <- List_erase_range_2( x, y )
    expect_equal( res[[1L]], list( 1L ) )
    expect_equal( res[[2L]], list("a" = 1L ) )

    expect_equal( integer_erase2(1:4), c(1L, 3L, 4L))
    ...
    expect_equal( integer_erase2(x), target)

    x <- 1:10
    expect_equal( integer_fill(x), rep(10L, 10 ))
